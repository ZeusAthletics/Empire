...
import { getCanonicalFacePrompt, listIdentityRefs } from "@/server/domain/nyx/identity/repository";

export default async function AdminNyxIdentityPage() {
  const scoped = await adminScoped("/admin/nyx-identity");
  const [refs, gallery, facePrompt] = await Promise.all([
    listIdentityRefs(),
    listNyxGallery(scoped.id),
    getCanonicalFacePrompt(),
  ]);
  return <NyxIdentityView refs={refs} gallery={gallery} facePrompt={facePrompt} />;
}
