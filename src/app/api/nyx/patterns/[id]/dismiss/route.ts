...
import { rejectPatternProposal } from "@/server/domain/nyx/proposalRepository";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await rejectPatternProposal(player.id, id);
    const talk = await getNyxTalk(player.id);
    return NextResponse.json({ ok: true, talk });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Patroon kon niet worden genegeerd.";
